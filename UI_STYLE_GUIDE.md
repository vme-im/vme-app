...
- **主按钮**: 黑色背景 + 白色文字 + 粗边框。
- **次级按钮**: 白色背景 + 黑色文字 + 粗边框。
- **CTA (行动号召)**: 红色或黄色背景，字体加粗斜体。

```jsx
// 示例：VME50 按钮
<button className="border-2 border-black bg-kfc-red px-6 py-2 font-black text-white shadow-neo-sm active:shadow-none">
  VME 50
</button>
```

### 装饰元素 (Decorations)
- **贴纸**: 使用 `rotate-12` 或 `-rotate-6` 让标签产生倾斜感。
- **分割线**: 使用粗黑线 `border-b-4 border-black` 进行区域分割。
- **纹理**: 
  - `bg-kfc-newsprint`: 页面全局叠加噪点纹理。
  - `bg-halftone`: 卡片内点缀半调网格圆点。
